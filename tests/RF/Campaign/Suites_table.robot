...
Check disabled value cell
    [Setup]    Setup
    Given I go to campaign page    &{P5C4}
    Then I check table cell value    t-suite    1    8    ${P5C4S1.disabled}
    [Teardown]    Teardown

A row suite with more or equal 95% should have successfull color
    [Setup]    Setup
    Given I go to campaign page    &{P1C4}
    Then Page Should Contain Element    xpath=//tr[@id="tr-suite-${P1C4S1.id}" and @class="table-success"]
    [Teardown]    Teardown

A row suite between 80% and 95% should have warning color
    [Setup]    Setup
    Given I go to campaign page    &{P3C3}
    Then Page Should Contain Element    xpath=//tr[@id="tr-suite-${P3C3S1.id}" and @class="table-warning"]
    [Teardown]    Teardown

A row suite under 80% should have failed color
    [Setup]    Setup
    Given I go to campaign page    &{P4C3}
